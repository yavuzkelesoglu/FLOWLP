...
async function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (authReq.session.adminId) {
    authReq.adminId = authReq.session.adminId;
    return next();
  }
...
export async function registerRoutes(
  ...
  app.post("/api/leads", async (req, res) => {
    try {
      ...
      // Enforce reCAPTCHA verification when secret key is configured
  ...
  // Get all leads (for admin purposes) - protected
  ...
  // Get notification emails setting - protected
  ...
  // Update notification emails setting - protected
